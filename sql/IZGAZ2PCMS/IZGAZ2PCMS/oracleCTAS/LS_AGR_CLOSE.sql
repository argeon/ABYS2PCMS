...
-- =====================================================================
DROP TABLE LS_AGR_CLOSE PURGE;


 CREATE TABLE LS_AGR_CLOSE AS
SELECT
...
CASE
    WHEN agr.REFUND_NUMBER > 0         THEN 5   -- Tamamlandı
    WHEN aca.WORK_ORDER_ID IS NULL     THEN 2   -- Mahsuplaşma Bekliyor
...
LEFT JOIN SMS.CS_REGISTER reg        ON reg.ID = agr.benefited_regıster_id        
