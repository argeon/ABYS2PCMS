...
BEGIN
    ...
    IF @SrcHasLrefIndex = 0 AND @RangeMode = 'KEYSET' AND @RaiseOnBlocking = 1
        RAISERROR(N'izgazMGR.dbo.LS_INVLINES uzerinde LREF onde index YOK. KEYSET modda her batch full scan olur (350M satir x 3500 batch). Once index kur: CREATE NONCLUSTERED INDEX IX_MIG_LSINVLINES_LREF ON izgazMGR.dbo.LS_INVLINES(LREF) WITH (MAXDOP=8, ONLINE=OFF, SORT_IN_TEMPDB=ON); ya da @RANGE_MODE=''ARITHMETIC'' kullan (insert taramasi yine index ister, sadece boundary sorgusu kalkar).', 16, 1);
