...
BEGIN
    ...
    SET @Sql = N'';
    SELECT @Sql = @Sql + N'
ALTER INDEX ' + QUOTENAME(i.name) + N' ON energy.dbo.LS_005_01_PAYTRANS REBUILD WITH (MAXDOP = 8, SORT_IN_TEMPDB = ON);'
    FROM sys.indexes i
    ...
    -- 3) Borç PT: invoice basina 1 (IOCODE=0 + ABYS) — TAKSIT SPLIT ile uyumsuz.
    --    Split ayni INVOICEREF icin coklu INST_NR>0 borc uretir; CANCELED filtrede ISNULL yasak.
    --    TODO: uygun unique tasarimi (orn. (INVOICEREF, INST_NR) + CANCELED=0) sonra.
    IF NOT EXISTS (
        SELECT 1 FROM sys.indexes
        WHERE object_id = OBJECT_ID('energy.dbo.LS_005_01_PAYTRANS')
    ...
    BEGIN
        RAISERROR('WARN SKIP UX_LS005_PAYTRANS_DEBT_INVOICEREF (taksit split uyumu; PIPELINE_KNOWN_ISSUES)', 0, 1) WITH NOWAIT;
    END
