/* ============================================================
   ...
   ============================================================ */
-- ============================================================
-- izgazMGR.dbo.LS_KFACTOR_VALUE → energy.dbo.LS_005_FUID
--   (CTAS: SMS.ME_FACTOR_VALUE → MIGRATION.LS_KFACTOR_VALUE → dump)
--   LREF    ← ID
--   VAL     ← HIGHER_HEATING_VALUE
--   DATE_   ← ACTION_DATE
--   RMSID   ← LS_005_RMS.LREF (SERVICE_BOX_ID = DEFN; 04_STATION seed)
...
BEGIN
    ...
    WHERE NOT EXISTS (
        ...
        WHERE s.name = 'dbo'
          AND t.name = 'LS_KFACTOR_VALUE'
          AND c.name = r.COL_NAME
    );

    IF @Missing IS NOT NULL AND @Missing <> N''
    BEGIN
        DECLARE @Msg NVARCHAR(4000) = N'LS_KFACTOR_VALUE eksik kolonlar: ' + @Missing;
        IF @RaiseOnMissing = 1
            RAISERROR(@Msg, 16, 1);
        ELSE
    ...
    IF @RaiseOnMissing = 0
        SELECT N'LS_KFACTOR_VALUE kaynak dogrulama OK' AS VALIDATION_MESSAGE;
...
FROM izgazMGR.dbo.LS_KFACTOR_VALUE s
