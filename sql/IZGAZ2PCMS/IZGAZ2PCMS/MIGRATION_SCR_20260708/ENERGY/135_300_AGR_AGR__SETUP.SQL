...
--   LREF    : kaynak LREF (KUL=CS_AGREEMENT.ID; ABN=MAX+seq)
--             bulk: 302_AGR_AGR__bulk_migrate.sql (IDENTITY_INSERT)
--   ABYS_ID : kaynak değeri aynen (KUL+ABN birleşimi; TP2 ile ayırt)
--   Köprü   : MIG_ROW_ID (ADDDATE, TP2, ABYS_ID sırası) + LREF
...
-- ABN fan-out: ayni TP2+ABYS_ID birden fazla LREF olabilir (tesisat coklama)
-- Unique degil; lookup icin non-unique index.
IF EXISTS (
    SELECT 1 FROM sys.indexes
    WHERE object_id = OBJECT_ID('energy.dbo.LS_005_01_AGR')
...
BEGIN
    ...
    BEGIN
        RAISERROR('LS_AGREEMENT: MIG_ROW_ID veya _MIG_UID yok. Once 300_AGR_AGR__setup.sql kolon adimlarini calistirin.', 16, 1);
        RETURN;
    END

    IF COL_LENGTH('izgazMGR.dbo.LS_AGREEMENT', 'LREF') IS NULL
    BEGIN
        RAISERROR('LS_AGREEMENT: LREF yok. Oracle CTAS + dump (LREF kolonlu) gerekli.', 16, 1);
        RETURN;
    END

    IF EXISTS (
        SELECT 1 FROM izgazMGR.dbo.LS_AGREEMENT WITH (NOLOCK)
        WHERE LREF IS NULL
           OR TRY_CAST(LREF AS BIGINT) NOT BETWEEN 1 AND 2147483647
    )
    BEGIN
        RAISERROR('LS_AGREEMENT: LREF NULL/gecersiz satir var. CTAS LREF atamasini kontrol edin.', 16, 1);
        RETURN;
    END
    ...
    IF EXISTS (
        SELECT 1 FROM izgazMGR.sys.indexes
        WHERE object_id = OBJECT_ID('izgazMGR.dbo.LS_AGREEMENT')
          AND name = 'UX_MIG_LSAGR_LREF'
    )
        DROP INDEX UX_MIG_LSAGR_LREF ON izgazMGR.dbo.LS_AGREEMENT;

    CREATE UNIQUE NONCLUSTERED INDEX UX_MIG_LSAGR_LREF
        ON izgazMGR.dbo.LS_AGREEMENT (LREF)
        WITH (MAXDOP = 24);
END
GO
...
BEGIN
    ...
    ;WITH Required (COL_NAME) AS (
        SELECT v.COL_NAME FROM (VALUES
            ('LREF'), ('MIG_ROW_ID'), ('TP2'), ('ABYS_ID'), ('FLATID'), ('CON'),
