...
BEGIN
    ...
    IF OBJECT_ID('tempdb..#CANCEL_PLAN') IS NOT NULL DROP TABLE #CANCEL_PLAN;
    CREATE TABLE #CANCEL_PLAN (
        PLAN_ID BIGINT NOT NULL PRIMARY KEY
    );

    INSERT INTO #CANCEL_PLAN (PLAN_ID)
    ...
    BEGIN
        ...
        WHERE ins.AGREEMENT_ID = @AGR_ID
          ...
          AND NOT EXISTS (
                SELECT 1 FROM #CANCEL_PLAN c WHERE c.PLAN_ID = CAST(ins.ID AS BIGINT)
              );
    END

    SET @N = (SELECT COUNT(*) FROM #CANCEL_PLAN);
    IF @DEBUG = 1
    BEGIN
    ...
    IF OBJECT_ID('tempdb..#INV_CANCEL') IS NOT NULL DROP TABLE #INV_CANCEL;
    CREATE TABLE #INV_CANCEL (
        INV_LREF INT NOT NULL PRIMARY KEY,
        PLAN_ID  BIGINT NOT NULL
    );

    INSERT INTO #INV_CANCEL (INV_LREF, PLAN_ID)
    SELECT inv.LREF, CAST(inv.ABYS_INSTALLMENT_ID AS BIGINT)
    FROM energy.dbo.LS_005_01_INVOICE inv WITH (NOLOCK)
    INNER JOIN #CANCEL_PLAN c ON c.PLAN_ID = inv.ABYS_INSTALLMENT_ID
    ...
    FROM energy.dbo.LS_005_01_INVOICE inv WITH (NOLOCK)
    INNER JOIN #CANCEL_PLAN c ON c.PLAN_ID = inv.INSTALLMENT_PLAN_REF
    WHERE inv.ABYS_AGREEMENT_ID = @AGR_ID
      AND ISNULL(inv.IOCODE, 0) = 0
      AND inv.INSTALLMENT_PLAN_REF IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM #INV_CANCEL x WHERE x.INV_LREF = inv.LREF);

    SET @N = (SELECT COUNT(*) FROM #INV_CANCEL);
    IF @DEBUG = 1
    BEGIN
    ...
    IF OBJECT_ID('tempdb..#MAIN_PT') IS NOT NULL DROP TABLE #MAIN_PT;
    SELECT
        i.INV_LREF,
        i.PLAN_ID,
    ...
    INNER JOIN #INV_CANCEL i ON i.INV_LREF = pt.INVOICEREF
    ...
    SET pt.CANCELED = 0,
        pt.PAID = 0
    FROM energy.dbo.LS_005_01_PAYTRANS pt
    INNER JOIN #MAIN_PT m ON m.MAIN_PT_LREF = pt.LREF;
    ...
    INNER JOIN #INV_CANCEL i ON i.INV_LREF = inv.LREF
    ...
    INNER JOIN #CANCEL_PLAN c ON c.PLAN_ID = pl.PLAN_ID
    ...
    FROM energy.dbo.LS_005_01_INVOICE inv
    INNER JOIN #MAIN_PT m ON m.INV_LREF = inv.LREF
...
BEGIN
    ...
    IF OBJECT_ID('tempdb..#CS_CANCEL') IS NOT NULL DROP TABLE #CS_CANCEL;
    CREATE TABLE #CS_CANCEL (ID BIGINT NOT NULL PRIMARY KEY);
    IF OBJECT_ID('izgazMGR.dbo.CS_INSTALLMENT', 'U') IS NOT NULL
        INSERT INTO #CS_CANCEL (ID)
        SELECT DISTINCT CAST(ins.ID AS BIGINT)
        FROM izgazMGR.dbo.CS_INSTALLMENT ins WITH (NOLOCK)
        WHERE ins.AGREEMENT_ID = @AGR_ID
          AND (
    ...
    IF OBJECT_ID('tempdb..#ACTIVE_PL') IS NOT NULL DROP TABLE #ACTIVE_PL;
    SELECT
    ...
    WHERE pl.ABYS_AGREEMENT_ID = @AGR_ID
      ...
      AND NOT EXISTS (
            SELECT 1 FROM #CS_CANCEL c
            WHERE c.ID = ISNULL(pl.ABYS_INSTALLMENT_ID, pl.PLAN_ID)
          );

    IF NOT EXISTS (SELECT 1 FROM #ACTIVE_PL)
        RETURN;

    IF OBJECT_ID('tempdb..#PLAN_SUM') IS NOT NULL DROP TABLE #PLAN_SUM;
    CREATE TABLE #PLAN_SUM (
        PLAN_KEY BIGINT NOT NULL PRIMARY KEY,
        SUM_AMT  DECIMAL(18,2) NOT NULL
    );

    INSERT INTO #PLAN_SUM (PLAN_KEY, SUM_AMT)
    ...
    FROM energy.dbo.LS_005_01_INVOICE inv WITH (NOLOCK)
    INNER JOIN #PLAN_SUM ps ON ps.PLAN_KEY = inv.ABYS_INSTALLMENT_ID
    WHERE inv.ABYS_AGREEMENT_ID = @AGR_ID
      AND inv.ABYS_INSTALLMENT_ID IS NOT NULL
      AND ISNULL(inv.IOCODE, 0) = 0
    ...
    SELECT
        ...
        inv.ABYS_ID, inv.ABYS_ACCOUNT_ID, inv.ABYS_ACTION_TYPE_ID, inv.ABYS_ACCRUE_TYPE_ID, inv.ABYS_REGISTER_ID
    FROM energy.dbo.LS_005_01_INVOICE inv WITH (NOLOCK)
    INNER JOIN #PLAN_SUM ps ON ps.PLAN_KEY = inv.INSTALLMENT_PLAN_REF
    WHERE inv.ABYS_AGREEMENT_ID = @AGR_ID
      AND inv.INSTALLMENT_PLAN_REF IS NOT NULL
      AND ISNULL(inv.IOCODE, 0) = 0
      AND ISNULL(inv.CANCELED, 0) = 0
      AND ISNULL(inv.PAYABLETOTAL, 0) > 0.01
      AND ABS(ISNULL(inv.PAYABLETOTAL, 0) - ps.SUM_AMT) <= 1.00
      AND NOT EXISTS (SELECT 1 FROM #INV x WHERE x.INV_LREF = inv.LREF)
      AND NOT EXISTS (
            SELECT 1
            FROM energy.dbo.LS_005_01_PAYTRANS p WITH (NOLOCK)
            WHERE p.INVOICEREF = inv.LREF
    ...
    IF OBJECT_ID('tempdb..#OLD_PT') IS NOT NULL DROP TABLE #OLD_PT;
    SELECT
        i.INV_LREF,
        i.PLAN_ID,
    ...
    SET pt.CANCELED = 1
    FROM energy.dbo.LS_005_01_PAYTRANS pt
    INNER JOIN #OLD_PT o ON o.OLD_PT_LREF = pt.LREF;
    ...
    ;WITH plan_rows AS (
        ...
        FROM #INV i
        INNER JOIN #OLD_PT o ON o.INV_LREF = i.INV_LREF
    ...
    FROM energy.dbo.LS_005_01_INSTALLMENT_PLAN pl
    INNER JOIN #NEW n ON n.PLAN_LREF = pl.LREF
    INNER JOIN energy.dbo.LS_005_01_PAYTRANS pt
    ...
    FROM energy.dbo.LS_005_01_INVOICE inv
    INNER JOIN #INV i ON i.INV_LREF = inv.LREF
    ...
    FROM energy.dbo.LS_005_01_PAYTRANS pay
    INNER JOIN #OLD_PT o ON o.OLD_PT_LREF = pay.CROSSREF
    CROSS APPLY (
