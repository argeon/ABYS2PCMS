...
<#
  20260708 snapshot yenileme (COPY only — kaynaklari kesmez/tasimaz).
  Kullanim: pwsh -File sql\IZGAZ2PCMS\IZGAZ2PCMS\BACKUP\20260708\_sync_snapshot.ps1

  Kaynak kokleri (2026-08 duzen):
    - prodEnergy  -> paket koku (IZGAZ2PCMS/IZGAZ2PCMS)
    - oracleCTAS* / ProdIzgazMgr2Energy -> BACKUP/
#>
$ErrorActionPreference = 'Stop'
$Snap = $PSScriptRoot
$BackupRoot = Split-Path $Snap -Parent
$PkgRoot = Split-Path $BackupRoot -Parent
if (-not (Test-Path (Join-Path $PkgRoot 'prodEnergy')) -or -not (Test-Path (Join-Path $BackupRoot 'oracleCTAS3007'))) {
    ...
    $Snap = Join-Path $BackupRoot '20260708'
}
# Geriye uyum: CTAS/Mgr yollari icin $Base = BACKUP
$Base = $BackupRoot
...
# ENERGY eksikler — dokuman / FRK test / 91d / hotfix
$energyAdd = @(
