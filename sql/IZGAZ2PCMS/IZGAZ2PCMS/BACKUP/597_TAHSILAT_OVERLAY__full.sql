...
BEGIN
    ...
    INSERT INTO energy.dbo.LS_005_01_PAYTRANS WITH (TABLOCK) (
        ...
        DV, PAYABLETOTAL, ADDDATE, ADDUSER, CANCELLATIONPAYMENT, BN_TYPE, XTYPE, PAYCURID,
    ...
    SELECT
        ...
        CAST(0 AS FLOAT), /* DV — LS_001: tahsilat DV genelde 0; 109 heal sonrasi BORC'tan gelebilir */
        CONVERT(FLOAT, CONVERT(DECIMAL(18,2), s.PAYABLETOTAL)),
        energy.dbo.FN_SAFE_SMALLDT_DEP(CAST(s.DATE_ AS DATETIME2)),
        CAST(ISNULL(main.ADDUSER, 20001) AS INT),
        CAST(0 AS INT),
        TRY_CAST(main.BN_TYPE AS INT),
        /* XTYPE = tahsilat kanali (O30 PayType); yoksa fatura/1 */
        CAST(COALESCE(TRY_CAST(s.XTYPE AS INT), TRY_CAST(main.XTYPE AS INT), 1) AS INT),
        CAST(ISNULL(main.CURID, 160) AS INT),
