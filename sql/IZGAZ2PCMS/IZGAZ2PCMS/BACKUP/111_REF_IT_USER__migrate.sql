...
-- ============================================================
-- SP_MIGRATE_IT_USER
-- Kaynak: izgazMGR.dbo.LS_IT_USER → Hedef: energy.dbo.LS_USER
...
BEGIN
    ...
    SELECT @SourceCount  = COUNT_BIG(*) FROM izgazMGR.dbo.LS_IT_USER;
    SELECT @MaxBridgeKey = MAX(ID)       FROM izgazMGR.dbo.LS_IT_USER;
    ...
    BEGIN
        RAISERROR('LS_IT_USER offset cakismasi: native LS_USER.USERID 10000+ bantinda. USERID=ABYS_ID+10000 guvensiz.', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        ...
        BEGIN TRY
            ...
            SELECT TOP (@BATCH_SIZE)
                ...
                END,
                u.WORK_PLACE_ID
            FROM izgazMGR.dbo.LS_IT_USER u
            LEFT JOIN izgazMGR.dbo.LS_REGISTER r
            ...
            BEGIN
                BEGIN TRY
                    ...
                    SELECT
                        ...
                        u.WORK_PLACE_ID
                    FROM izgazMGR.dbo.LS_IT_USER u
                    LEFT JOIN izgazMGR.dbo.LS_REGISTER r ON r.ID = u.REGISTER_ID
