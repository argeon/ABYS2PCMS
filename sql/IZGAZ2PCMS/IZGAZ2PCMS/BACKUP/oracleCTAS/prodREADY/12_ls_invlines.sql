...
-- prodREADY: LINENR + LINENR_SRC (ayni RN). Energy 581 CAST SMALLINT sart.
-- =============================================================================
...
SELECT /*+ PARALLEL(56) */
    ...
    /* LINENR = sira (energy SMALLINT); LINENR_SRC = kayipsiz ayni RN (TINYINT clamp YOK) */
    CAST(
      ROW_NUMBER() OVER (
...
SELECT
  ...
  (SELECT COUNT(*) FROM (
      ...
      GROUP BY INVOICEREF HAVING MAX(LINENR) > 255
   )) AS INV_LINENR_GT255
FROM DUAL;

DECLARE
  n_line NUMBER; n_bad NUMBER; n_gt255 NUMBER;
BEGIN
  ...
  MIGRATION.P_MIG_CTAS_LOG('O12', 'ls_invlines', 'OK', n_line,
    'bad_lref=0 linenr_gt255_inv=' || n_gt255
    || CASE WHEN n_gt255 > 0 THEN ' ★ energy 01_linenr_smallint ZORUNLU' ELSE '' END);
  DBMS_OUTPUT.PUT_LINE('========== O12 LS_INVLINES OK | lines=' || n_line
    || ' | LINENR>255 fatura=' || n_gt255 || ' ==========');
END;
/
