-- =============================================================================
-- prod2 / 30 — TAHSILAT OVERLAY (PAYTRANS zinciri)
-- Ortam : Oracle 11.2 | Schema: MIGRATION | Kaynak: SMS
-- Onkosul: 11 LS_INVOICE + 20 eksilten (LS_OV_EKS_CLASS / IADE)
-- MOD: FULL — TMP_MIG_ACC = tum hesaplar ACCRUE<>14
-- DOP: FORCE 56 | index PARALLEL 56
-- RISK: sentetik LREF 1.35B/1.8B/1.9B+EKS INT; PAY_MULTI soft; Tip12 emanet overlay disi
-- =============================================================================
...
--   Arttiran: RN_TYPE per ACTION_TYPE (1+3+41+10) → MAIN_PRI waterfill cascade
-- =============================================================================
...
picked AS (
    SELECT
        ...
        /* Tip basina 1 MAIN (1=tahakkuk, 3=arttiran, 41/10=gecikme)
           Eski RN=1 yalniz tip1 aliyordu → arttiran hic ALLOC gormuyordu.
           Waterfill (MAIN_PRI) tip1 dolunca tip3'e aktarir — cift sayim yok. */
        ROW_NUMBER() OVER (
...
CREATE UNIQUE INDEX MIGRATION.IX_OV_CANCEL_PAY ON MIGRATION.LS_OV_CANCEL_PAY (LREF) PARALLEL 56 NOLOGGING;
...
CREATE UNIQUE INDEX MIGRATION.IX_OV_CANCEL_REV ON MIGRATION.LS_OV_CANCEL_REV (LREF) PARALLEL 56 NOLOGGING;
...
CREATE UNIQUE INDEX MIGRATION.IX_OV_MAHSUP_SRC ON MIGRATION.LS_OV_MAHSUP_SRC (PAY_LREF) PARALLEL 56 NOLOGGING;
...
CREATE UNIQUE INDEX MIGRATION.IX_OV_MAHSUP_CLOSED ON MIGRATION.LS_OV_MAHSUP_CLOSED (MAIN_LREF) PARALLEL 56 NOLOGGING;
ALTER INDEX MIGRATION.IX_OV_MAHSUP_CLOSED NOPARALLEL;
/
CREATE INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_DEP ON MIGRATION.LS_OV_MAHSUP_CLOSED (DEP_ACCOUNT_ID) PARALLEL 56 NOLOGGING;
ALTER INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_DEP NOPARALLEL;
/
CREATE INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_KIND ON MIGRATION.LS_OV_MAHSUP_CLOSED (CLOSE_KIND) PARALLEL 56 NOLOGGING;
ALTER INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_KIND NOPARALLEL;
/
...
CREATE INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_AGR ON MIGRATION.LS_OV_MAHSUP_CLOSED (ABYS_AGREEMENT_ID) PARALLEL 56 NOLOGGING;
...
ALTER INDEX MIGRATION.IX_OV_MAHSUP_CLOSED_AGR NOPARALLEL;

-- =============================================================================
-- 5) LOG — AYRI DOSYA: 40_ls_tahsilat_log.sql
-- 6) HARD GATE — AYRI DOSYA: 41_gate_tahsilat.sql
-- =============================================================================
PROMPT ========== 30 TAHSILAT OVERLAY OK — sonraki: 40 log + 41 gate ==========
/