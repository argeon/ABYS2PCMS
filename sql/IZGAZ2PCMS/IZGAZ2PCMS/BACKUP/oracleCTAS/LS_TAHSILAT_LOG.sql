-- =============================================================================
-- ADIM 5b — TAHSILAT LOG (DIAGNOSTIC ONLY — ana overlay'den AYRI)
-- Dosya : oracleCTAS/LS_TAHSILAT_LOG.sql
-- Ortam : Oracle 11.2 | Schema: MIGRATION
--
-- Onkosul: LS_TAHSILAT_OVERLAY.sql basariyla bitti
--          (TMP_MIG_ACC, TMP_PAY_CANCEL, LS_OV_PAY_ALLOC/PT/DEBT/MAHSUP)
--
-- AMAC: tip12 atlama, PAY_NO_ALLOC, multi-emanet vb. GOZLEMLEMEK
--       Ana ALLOC/PAY_PT/DEBT uretimine DOKUNMAZ.
--
-- Kullanim: overlay KOMPLE bittikten SONRA ayri kosulur.
--           Log hata verse bile production tablolari bozulmaz.
-- =============================================================================
-- =============================================================================
-- 5) LOG — atlanan / uyarilan tahsilat durumlari
-- REASON:
--   NO_XREF             — PAY_PT'de CROSSREF yok (legacy; alloc-only PT'de nadir)
--   PAY_CANCELED        — R42 tahsilat iptali (CANCELED=1)
--   MAIN_MISSING        — xref var ama LS_INVOICE'da MAIN yok
--   DEBT_PARTIAL        — PAID guncellendi ama CLOSED=0 (kismi odeme)
--   PAY_NO_ALLOC        — tip2 odeme (iptal degil) ALLOC'a dusmedi > PT yok
--   EMANET_CIKIS_SKIP   — tip12 emanet cikisi (ATP.TYPE=7) overlay disi
--   EMANET_CIKIS_MATCH  — tip12 + borc INCOME eslesmesi VAR (dahil edilse alloc olurdu)
--   MAHSUP_NO_DEP       — mahsup PT ama REF_DEPOSIT_ACCOUNT_ID bos
--   EMANET_MULTI_MAIN   — ayni emanet hesabi birden fazla MAIN kapatti
--   DEP_ACCRUE_ODD      — mahsup DepAcc ACCRUE_TYPE <> 14 (beklenen emanet degil)
-- =============================================================================
