...
-- Hedef  : SMS_AUDIT.LS_PROJECT  →  izgazMGR.dbo.LS_PROJECT
...
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE SMS_AUDIT.LS_PROJECT PURGE';
...
CREATE TABLE SMS_AUDIT.LS_PROJECT
NOLOGGING
AS
SELECT
    SMS_AUDIT.SEQ_LS_PROJECT.NEXTVAL                            AS LREF,
    ...
    SUBSTR(
        ...
        || '-'
        || TO_CHAR(SMS_AUDIT.SEQ_LS_PROJECT.CURRVAL)
    , 1, 50)                                                    AS CODE,
...
ALTER TABLE SMS_AUDIT.LS_PROJECT NOPARALLEL LOGGING;

CREATE UNIQUE INDEX SMS_AUDIT.UX_LS_PROJECT_LREF
    ON SMS_AUDIT.LS_PROJECT (LREF) NOLOGGING PARALLEL 8;
ALTER INDEX SMS_AUDIT.UX_LS_PROJECT_LREF NOPARALLEL;

-- fan-out mumkun (coklu BNA.CODE) → UNIQUE degil
CREATE INDEX SMS_AUDIT.IDX_LS_PROJECT_ORACLE_ID
    ON SMS_AUDIT.LS_PROJECT (ORACLE_PROJECT_ID) NOLOGGING PARALLEL 8;
ALTER INDEX SMS_AUDIT.IDX_LS_PROJECT_ORACLE_ID NOPARALLEL;

CREATE INDEX SMS_AUDIT.IDX_LS_PROJECT_FIRM
    ON SMS_AUDIT.LS_PROJECT (FIRM_ID) NOLOGGING PARALLEL 8;
ALTER INDEX SMS_AUDIT.IDX_LS_PROJECT_FIRM NOPARALLEL;

CREATE INDEX SMS_AUDIT.IDX_LS_PROJECT_TYPE
    ON SMS_AUDIT.LS_PROJECT (PROJECT_TYPE_ID) NOLOGGING PARALLEL 8;
ALTER INDEX SMS_AUDIT.IDX_LS_PROJECT_TYPE NOPARALLEL;

CREATE INDEX SMS_AUDIT.IDX_LS_PROJECT_STAT
    ON SMS_AUDIT.LS_PROJECT (STATID) NOLOGGING PARALLEL 8;
ALTER INDEX SMS_AUDIT.IDX_LS_PROJECT_STAT NOPARALLEL;

BEGIN
  DBMS_STATS.GATHER_TABLE_STATS('SMS_AUDIT', 'LS_PROJECT', degree => 56);
END;
/
...
-- SELECT COUNT(1) AS TGT_CNT FROM SMS_AUDIT.LS_PROJECT;
-- SELECT STATID, COUNT(1) FROM SMS_AUDIT.LS_PROJECT GROUP BY STATID ORDER BY 1;
-- SELECT PROJECT_TYPE_ID, COUNT(1) FROM SMS_AUDIT.LS_PROJECT GROUP BY PROJECT_TYPE_ID ORDER BY 1;
--
