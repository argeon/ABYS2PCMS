...
-- ------------------------------------------------------------
-- Tahsilat zinciri: izgazMGR.LS_INVLINES KEYSET / join IX
-- Yoksa CREATE (355M+ — uzun surebilir). 581 basinda cagrilir.
-- ------------------------------------------------------------
CREATE OR ALTER PROCEDURE dbo.SP_MIG_INVLINES_ENSURE_MGR_INDEXES
    @DEBUG BIT = 1
AS
BEGIN
    SET NOCOUNT ON;
    ...
    IF NOT EXISTS (
        SELECT 1
        FROM izgazMGR.sys.indexes
        WHERE object_id = OBJECT_ID('izgazMGR.dbo.LS_INVLINES')
          AND name = N'IX_MIG_LSINVLINES_LREF'
    )
    BEGIN
        IF @DEBUG = 1
            RAISERROR('CREATE IX_MIG_LSINVLINES_LREF ON izgazMGR.LS_INVLINES(LREF) — uzun surebilir...', 0, 1) WITH NOWAIT;
        ...
        IF @DEBUG = 1
            RAISERROR('OK IX_MIG_LSINVLINES_LREF', 0, 1) WITH NOWAIT;
    END
    ELSE IF @DEBUG = 1
        RAISERROR('SKIP IX_MIG_LSINVLINES_LREF (exists)', 0, 1) WITH NOWAIT;

    IF NOT EXISTS (
        SELECT 1
        FROM izgazMGR.sys.indexes
        WHERE object_id = OBJECT_ID('izgazMGR.dbo.LS_INVLINES')
          AND name = N'IX_MIG_LSIL_INVREF'
    )
    BEGIN
        IF @DEBUG = 1
            RAISERROR('CREATE IX_MIG_LSIL_INVREF ON izgazMGR.LS_INVLINES(INVOICEREF) — uzun surebilir...', 0, 1) WITH NOWAIT;
        CREATE NONCLUSTERED INDEX IX_MIG_LSIL_INVREF
            ON izgazMGR.dbo.LS_INVLINES (INVOICEREF)
            INCLUDE (LREF, GRANDTOTAL, ABYS_INCOME_ROW_ID)
            WITH (MAXDOP = 24, ONLINE = OFF, SORT_IN_TEMPDB = ON);
        IF @DEBUG = 1
            RAISERROR('OK IX_MIG_LSIL_INVREF', 0, 1) WITH NOWAIT;
    END
    ELSE IF @DEBUG = 1
        RAISERROR('SKIP IX_MIG_LSIL_INVREF (exists)', 0, 1) WITH NOWAIT;
END
GO
...
PRINT '580_INVLINES__setup OK (FK/index yok — post: 582; ENSURE_MGR_INDEXES)';
GO
