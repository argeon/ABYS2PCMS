/* ============================================================
   SCRIPT_ID : TAHSILAT_OVERLAY_PILOT
   SCRIPT_NO : 597
   FILE      : 597_TAHSILAT_OVERLAY__pilot_agr.sql
   VERSION   : 1
   ============================================================ */
...
BEGIN
    ...
    IF @AGR_ID IS NULL
    BEGIN
        RAISERROR('SP_MIG_TAHSILAT_OVERLAY_CLEAN_BY_AGR: @AGR_ID zorunlu.', 16, 1);
        RETURN;
    END
    ...
    IF OBJECT_ID('izgazMGR.dbo.LS_OV_PAY_PT', 'U') IS NOT NULL
        ...
        WHERE TRY_CAST(ABYS_AGREEMENT_ID AS BIGINT) = @AGR_ID
    ...
    IF OBJECT_ID('izgazMGR.dbo.LS_OV_CANCEL_PAY', 'U') IS NOT NULL
        ...
        WHERE c.ABYS_AGREEMENT_ID = @AGR_ID AND c.LREF BETWEEN 1 AND 2147483647
    ...
    IF OBJECT_ID('izgazMGR.dbo.LS_OV_CANCEL_REV', 'U') IS NOT NULL
        ...
        WHERE r.ABYS_AGREEMENT_ID = @AGR_ID AND r.LREF BETWEEN 1 AND 2147483647
...
BEGIN
    ...
    WHERE TRY_CAST(s.ABYS_AGREEMENT_ID AS BIGINT) = @AGR_ID
    ...
    BEGIN
        ...
        ;WITH debt_cnt AS (
            ...
            WHERE TRY_CAST(u.ABYS_AGREEMENT_ID AS BIGINT) = @AGR_ID
              AND ISNULL(d.IOCODE, 0) = 0
              AND ISNULL(d.CANCELLATIONPAYMENT, 0) = 0
        ...
        pay_sum AS (
            ...
            WHERE ISNULL(p.IOCODE, 0) = 1
              ...
              AND TRY_CAST(p.ABYS_AGREEMENT_ID AS BIGINT) = @AGR_ID
            GROUP BY p.CROSSREF
        )
        UPDATE pt
        SET pt.PAID = CASE
    ...
    BEGIN
        ...
        WHERE c.ABYS_AGREEMENT_ID = @AGR_ID
...
PRINT '597_TAHSILAT_OVERLAY__pilot_agr OK';
GO
