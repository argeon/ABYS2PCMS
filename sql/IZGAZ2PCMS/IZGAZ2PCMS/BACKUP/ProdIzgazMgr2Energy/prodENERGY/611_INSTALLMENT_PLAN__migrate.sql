...
BEGIN
    ...
    IF OBJECT_ID('dbo.MIG_611_STG_IP_KEYS', 'U') IS NULL
       OR OBJECT_ID('dbo.MIG_611_STG_IP_BATCH_KEYS', 'U') IS NULL
    BEGIN
        RAISERROR('MIG_611_STG_* yok — once 00e_taksit_staging.sql', 16, 1);
        RETURN;
    END

    TRUNCATE TABLE dbo.MIG_611_STG_IP_KEYS;

    /* AGR + batch keys -> equality; full/bisect -> BETWEEN */
    IF @AGR_ID IS NOT NULL
       AND EXISTS (SELECT 1 FROM dbo.MIG_611_STG_IP_BATCH_KEYS)
        INSERT INTO dbo.MIG_611_STG_IP_KEYS (ID)
        SELECT DISTINCT bk.ID FROM dbo.MIG_611_STG_IP_BATCH_KEYS bk
    ...
    FROM izgazMGR.dbo.CS_INSTALLMENT_PLAN ip
    INNER JOIN dbo.MIG_611_STG_IP_KEYS kx ON kx.ID = ip.ID
...
BEGIN
    ...
    /* v4: physical MIG_611_STG_* key-list */
    IF OBJECT_ID('dbo.MIG_611_STG_IP_BATCH_KEYS', 'U') IS NULL
    BEGIN
        RAISERROR('MIG_611_STG_IP_BATCH_KEYS yok — once 00e_taksit_staging.sql', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        ...
        TRUNCATE TABLE dbo.MIG_611_STG_IP_BATCH_KEYS;

        IF @AGR_ID IS NULL
            INSERT INTO dbo.MIG_611_STG_IP_BATCH_KEYS (ID)
        ...
        SELECT @BatchFrom = MIN(ID), @BatchTo = MAX(ID) FROM dbo.MIG_611_STG_IP_BATCH_KEYS;
        ...
        BEGIN
            SET @Msg = 'Batch ' + CAST(@BatchNo AS VARCHAR(10))
                + ' | keys=' + CAST((SELECT COUNT(*) FROM dbo.MIG_611_STG_IP_BATCH_KEYS) AS VARCHAR(20))
