...
-- =============================================================================
-- oracleCTAS3007 / 11 — LS_INVOICE (prodREADY + pilot)
-- Onkosul: 10_stg_inv_acc_inc.sql
--          09_mig_accrue_type_map.sql (TYPE/EXPLAIN)
--          LS_READING (+ LS_READING_SYNTH) — READ_TRANSREF = MAX(LREF) by account
-- LREF = ABYS_ACTION_ID = CS_ACCOUNT_ACTION.ID (571 IDENTITY_INSERT)
-- 3007: ABYS_INSTALLMENT_ID/PLAN_REF = NVL(acc,aa); XTYPE=1
-- TYPE ← MIG_ACCRUE_TYPE_MAP.PCMS_TYPE (+ action 93 fallback)
-- EXPLAIN ← EXISTING || ' ' || map PREFIX+FMT (sonda; overwrite yok)
...
SELECT /*+ PARALLEL(56) FULL(aa) FULL(acc) FULL(av) */
    ...
    /* Pilot: action.INSTALLMENT_ID sık NULL — hesap öncelikli */
    NVL(acc.INSTALLMENT_ID, aa.INSTALLMENT_ID)                    AS ABYS_INSTALLMENT_ID,
    ...
    CAST(
      CASE
        WHEN aa.ACTION_TYPE_ID IN (3, 10, 41) AND map.PCMS_TYPE IS NULL THEN 93
        WHEN map.PCMS_TYPE IS NOT NULL THEN map.PCMS_TYPE
        ELSE 119
      END AS NUMBER(3)
    )                                                             AS TYPE,
    ...
    /* tutar: NUMBER(18,3) — dump/migrator scale kaybini onler (ROUND tek basina tip vermez) */
    CAST(ROUND(COALESCE(av.TOTAL_EXCL_TAX, 0), 3) AS NUMBER(18,3)) AS TLTOTAL,
    CAST(160 AS NUMBER(10))                                       AS CURID,
    CAST(ROUND(COALESCE(av.TOTAL_EXCL_TAX, 0), 3) AS NUMBER(18,3)) AS CURTOTAL,

    acc.ACCRUE_TYPE_ID                                            AS ACCRUE_TYPE_ID,

    /* EXISTING + trailing map EXPLAIN (PREFIX + FMT_TES|FMT_GV); never overwrite */
    CAST(SUBSTR(TRIM(
      NVL(NULLIF(TRIM(SUBSTR(NVL(aa.DESCRIPTION, acc.DESCRIPTION), 1, 200)), ''), '')
      || CASE
           WHEN map.EXPLAIN_PREFIX IS NULL THEN ''
           WHEN map.EXPLAIN_FMT = 'GV' THEN
             CASE WHEN NULLIF(TRIM(SUBSTR(NVL(aa.DESCRIPTION, acc.DESCRIPTION), 1, 200)), '') IS NULL
                  THEN '' ELSE ' ' END
             || map.EXPLAIN_PREFIX
             || ' TESISAT NO : ' || TO_CHAR(acc.INSTALLATION_ID)
             || ', SOZLESME NO : ' || TO_CHAR(acc.AGREEMENT_ID)
           WHEN map.EXPLAIN_FMT = 'TES' THEN
             CASE WHEN NULLIF(TRIM(SUBSTR(NVL(aa.DESCRIPTION, acc.DESCRIPTION), 1, 200)), '') IS NULL
                  THEN '' ELSE ' ' END
             || map.EXPLAIN_PREFIX
             || ' Tes.No:' || TO_CHAR(acc.INSTALLATION_ID)
             || ' Söz.No:' || TO_CHAR(acc.AGREEMENT_ID)
             || ' Abn No:' || TO_CHAR(acc.REGISTER_ID)
           ELSE ''
         END
    ), 1, 250) AS VARCHAR2(250))                                  AS EXPLAIN,
    ...
    CAST(ROUND(COALESCE(av.GAS_OPEN_FEE, 0), 3) AS NUMBER(18,3))  AS gas_open_fee,
    CAST(0 AS NUMBER(18,3))                                       AS detach_attach_fee,
    CAST(0 AS NUMBER(18,3))                                       AS test_fee,
    CAST(ROUND(COALESCE(av.SPEC_SERV_FEE, 0), 3) AS NUMBER(18,3)) AS spec_serv_fee,
    CAST(0 AS NUMBER(18,3))                                       AS fixed_fee,
    CAST(ROUND(COALESCE(av.EXPEND_FEE, 0), 3) AS NUMBER(18,3))    AS expend_fee,
    CAST(ROUND(COALESCE(av.DEFAULT_FINE, 0), 3) AS NUMBER(18,3))  AS DEFAULT_FINE,
    CAST(ROUND(COALESCE(av.DEFAULT_FINE_TAX, 0), 3) AS NUMBER(18,3)) AS DEFAULT_FINETAX,
    ...
    CAST(ROUND(COALESCE(av.ILLEGAL_USE_FEE, 0), 3) AS NUMBER(18,3)) AS illegal_use_fee,

    CAST(NULL AS NUMBER(10))                                      AS LOGO_FIRMNR,
    CAST(NULL AS NUMBER(10))                                      AS LOGO_FICHEREF,
    CAST(NULL AS VARCHAR2(100))                                   AS LOGO_FICHENO,
    CAST(ROUND(COALESCE(av.DISCOUNT_ADDITION, 0), 3) AS NUMBER(18,3)) AS discount_addition,
    ...
    CAST(ROUND(COALESCE(av.PAYABLE_TOTAL, 0), 3) AS NUMBER(18,3)) AS AMOUNT,
    ...
    CAST(ROUND(COALESCE(av.DISCOUNT_AMOUNT, 0), 3) AS NUMBER(18,3)) AS DISCOUNT_AMOUNT,
...
LEFT JOIN MIGRATION.MIG_ACCRUE_TYPE_MAP map
  ON map.ABYS_ACCRUE_TYPE_ID = acc.ACCRUE_TYPE_ID
LEFT JOIN (
    SELECT ABYS_ACCOUNT_ID AS ACCOUNT_ID, MAX(LREF) AS READING_ID
...
WHERE aa.ACTION_TYPE_ID IN (1, 3, 10, 41)
  ...
  AND (
     ...
     OR acc.AGREEMENT_ID IN (SELECT p.AGR_ID FROM MIGRATION.MIG_PARAM p WHERE p.AGR_ID IS NOT NULL)
      )
...
-- Soft gate + log (src = CTAS ile ayni MIG_PARAM filtre)
SELECT
