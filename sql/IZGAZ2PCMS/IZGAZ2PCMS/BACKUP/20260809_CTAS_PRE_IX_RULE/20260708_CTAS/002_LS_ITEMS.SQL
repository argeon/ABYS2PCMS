...
--   LASTENDEX = COALESCE(NULLIF(tesisat,0), NULLIF(sayac,0), ...)
--   ABYS_MTR_LAST_INDEX = CS_METER.LAST_INDEX
--   ABYS_INS_LAST_INDEX = tesisat INDEX_TYPE_ID=1
--   UPDUSER / UPDDATE staging'de tip olarak yer degismis (240 notu); CTAS ayni semayi korur
--
-- DOP: FORCE PARALLEL 56 (CTAS + STATS)
-- =====================================================================
...
SELECT /*+ FULL(m) PARALLEL(m 56) */
    ...
    m.LAST_INDEX                                               AS ABYS_MTR_LAST_INDEX,
...
ALTER TABLE MIGRATION.LS_ITEMS NOPARALLEL LOGGING;

BEGIN
  DBMS_STATS.GATHER_TABLE_STATS('MIGRATION', 'LS_ITEMS', degree => 56);
END;
/
...
-- SELECT METER_ID, COUNT(1) AS CNT
