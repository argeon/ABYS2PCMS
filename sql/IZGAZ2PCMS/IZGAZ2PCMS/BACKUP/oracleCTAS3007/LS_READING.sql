-- =====================================================================================
-- LS_READING CTAS - v04
-- Okuma: 4 stage + 1 MAIN — hepsi MIGRATION semasinda yasamli
--
--   MIGRATION.STG_RD_SYNC_USER
--   MIGRATION.STG_RD_USER_COMP
--   MIGRATION.STG_RD_READING_INC
--   MIGRATION.STG_RD_ACC_INC
--   MIGRATION.LS_READING          (MAIN; stage sonrasi)
--
-- Kaynak SMS.* tablolarina dokunulmaz (CS_READING, IT_USER, …).
-- HHD mstr + read_no: MAIN sonrasi @LS_HHD_MSTR.sql + @LS_HHD_MSTR_GATE.sql
...
-- =====================================================================================
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_SYNC_USER PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
END;
/
CREATE TABLE MIGRATION.STG_RD_SYNC_USER NOLOGGING PARALLEL 56 AS
SELECT /*+ PARALLEL(56) */
       SYNC_CLIENT_ID, USER_ID, USER_NAME
FROM (
...
ALTER TABLE MIGRATION.STG_RD_SYNC_USER NOPARALLEL LOGGING;
BEGIN DBMS_STATS.GATHER_TABLE_STATS('MIGRATION','STG_RD_SYNC_USER', degree => 56); END;
/
...
-- =====================================================================================
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_USER_COMP PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
END;
/
CREATE TABLE MIGRATION.STG_RD_USER_COMP NOLOGGING PARALLEL 56 AS
SELECT /*+ PARALLEL(56) */
       USER_ID, USER_NAME, COMP_REGISTER_ID
FROM (
...
ALTER TABLE MIGRATION.STG_RD_USER_COMP NOPARALLEL LOGGING;
BEGIN DBMS_STATS.GATHER_TABLE_STATS('MIGRATION','STG_RD_USER_COMP', degree => 56); END;
/
...
-- =====================================================================================
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_READING_INC PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
END;
/
CREATE TABLE MIGRATION.STG_RD_READING_INC NOLOGGING PARALLEL 56 AS
SELECT /*+ FULL(ri) PARALLEL(ri 56) */
    ri.READING_ID,
...
ALTER TABLE MIGRATION.STG_RD_READING_INC NOPARALLEL LOGGING;
BEGIN DBMS_STATS.GATHER_TABLE_STATS('MIGRATION','STG_RD_READING_INC', degree => 56); END;
/
...
-- =====================================================================================
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_ACC_INC PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
END;
/
CREATE TABLE MIGRATION.STG_RD_ACC_INC NOLOGGING PARALLEL 56 AS
...
ALTER TABLE MIGRATION.STG_RD_ACC_INC NOPARALLEL LOGGING;
BEGIN DBMS_STATS.GATHER_TABLE_STATS('MIGRATION','STG_RD_ACC_INC', degree => 56); END;
/
...
-- =====================================================================================
BEGIN
  EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.LS_READING PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF;
END;
/
CREATE TABLE MIGRATION.LS_READING
NOLOGGING
PARALLEL 56
AS
SELECT /*+ PARALLEL(56) */
    ...
    NVL(r.CONSUMPTION,
        ROUND(NVL((((r.CORRECTED_SM3 * r.ADJUSTMENT_COEFFICIENT) * r.HIGH_HEATING_VALUE) / 9155), 2),
              r.CONSUMPTION))                                   AS corr_volume,
...
LEFT JOIN MIGRATION.STG_RD_ACC_INC   av  ON av.ACCOUNT_ID = acc.ID
...
LEFT JOIN SMS.IT_ENUM_BUNDLE     eb  ON eb.ENUM_NAME = 'ReadingInformationTransferStatus'
                                    ...
                                    AND eb.LANG_ID = 1
LEFT JOIN MIGRATION.STG_RD_READING_INC pv ON pv.READING_ID = r.ID
LEFT JOIN MIGRATION.STG_RD_SYNC_USER   scu ON scu.SYNC_CLIENT_ID = r.TERMINAL_SYNC_CLIENT_ID
LEFT JOIN MIGRATION.STG_RD_USER_COMP   uc  ON uc.USER_ID =
...
ALTER TABLE MIGRATION.LS_READING NOPARALLEL LOGGING;

CREATE INDEX MIGRATION.IX_LS_READING_ABYS_ACCOUNT_ID
  ON MIGRATION.LS_READING (ABYS_ACCOUNT_ID) PARALLEL 56 NOLOGGING;
ALTER INDEX MIGRATION.IX_LS_READING_ABYS_ACCOUNT_ID NOPARALLEL;

CREATE INDEX MIGRATION.IX_LS_READING_ACCRUE_TYPE
  ON MIGRATION.LS_READING (ABYS_ACCRUE_TYPE_ID) PARALLEL 56 NOLOGGING;
ALTER INDEX MIGRATION.IX_LS_READING_ACCRUE_TYPE NOPARALLEL;

BEGIN DBMS_STATS.GATHER_TABLE_STATS('MIGRATION','LS_READING', degree => 56); END;
/

-- STG_DROP: MAIN tüketti — ara tabloları düş
BEGIN EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_SYNC_USER PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_USER_COMP PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_READING_INC PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE MIGRATION.STG_RD_ACC_INC PURGE';
EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/

PROMPT ========== LS_READING OK (ABYS_ACCRUE_TYPE_ID + STG_RD DROP) ==========
/
