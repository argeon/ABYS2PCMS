...
-- Kaynak  : izgazMGR.dbo.LS_KFACTOR_VALUE
--           (CTAS: SMS.ME_FACTOR_VALUE → LS_KFACTOR_VALUE dump)
...
BEGIN
    ...
    SELECT @SourceCount  = COUNT_BIG(*) FROM izgazMGR.dbo.LS_KFACTOR_VALUE;
    SELECT @MaxBridgeKey = MAX(ID)       FROM izgazMGR.dbo.LS_KFACTOR_VALUE;

    EXEC energy.dbo.SP_MIG_LOG_START_RUN
        @MigrationCode  = @MigrationCode,
        @SourceDb       = 'izgazMGR',
        @SourceTable    = 'LS_KFACTOR_VALUE',
    ...
    BEGIN
        ...
        FROM (
            ...
            FROM izgazMGR.dbo.LS_KFACTOR_VALUE
...
/****** Object:  Index [IX_LS_005_OFID]    Script Date: 26.07.2026 15:57:27 ******/
ALTER TABLE [dbo].[LS_005_FUID] DROP CONSTRAINT [IX_LS_005_OFID]
GO

/****** Object:  Index [IX_LS_005_OFID]    Script Date: 26.07.2026 15:57:27 ******/
ALTER TABLE [dbo].[LS_005_FUID] ADD  CONSTRAINT [IX_LS_005_OFID] UNIQUE NONCLUSTERED 
(
	[DATE_] ASC,
	[RMSID] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 50, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
GO
