/* ============================================================
   ...
   ============================================================ */
-- ============================================================
-- izgazMGR.dbo.LS_READING_197168
...
BEGIN
    ...
    IF OBJECT_ID('izgazMGR.dbo.LS_READING_197168', 'U') IS NULL
    BEGIN
        IF @RaiseOnMissing = 1
            RAISERROR('izgazMGR.dbo.LS_READING_197168 bulunamadi.', 16, 1);
        RETURN 1;
    END
    ...
    WHERE NOT EXISTS (
        ...
        WHERE s.name = 'dbo' AND t.name = 'LS_READING_197168' AND c.name = r.COL_NAME
    );

    IF @Missing IS NOT NULL AND @Missing <> N''
    BEGIN
        DECLARE @Msg NVARCHAR(4000) = N'LS_READING_197168 eksik kolonlar: ' + @Missing;
        IF @RaiseOnMissing = 1 RAISERROR(@Msg, 16, 1);
        RETURN 1;
    END

    IF @RaiseOnMissing = 0
        SELECT N'LS_READING_197168 kaynak dogrulama OK' AS VALIDATION_MESSAGE;
...
BEGIN
    ...
    IF @RaiseOnMissing = 0
        SELECT N'LS_READING_197168 kaynak + LS_005_01_hhd_loc_inv_tran hedef dogrulama OK' AS VALIDATION_MESSAGE;
...
SELECT
    ...
    s.PERIOD                                                                    AS PERIOD
FROM izgazMGR.dbo.LS_READING_197168 s
