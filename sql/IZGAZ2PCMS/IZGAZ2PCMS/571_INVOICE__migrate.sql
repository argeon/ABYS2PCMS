/* ============================================================
   ...
   VERSION   : 9
   ============================================================ */
-- v9: key-list batch — #MIG_INV_BATCH_KEYS join (BETWEEN seyrek AGR araligi YOK)
...
-- v2: batch/LREF = kaynak LREF; EXPLAIN yok → NULL; tip/uzunluk guncellemeleri
-- ============================================================
...
-- Batch: caller #MIG_INV_BATCH_KEYS doldurur; INSERT_RANGE equality join (BETWEEN yok).
...
BEGIN
    ...
    /* v10:
       - Full (@AGR_ID NULL): dogrudan BETWEEN — #MIG_INV_KEYS join/ROW_NUMBER cift tarama YOK
       - AGR: #MIG_INV_BATCH_KEYS equality (seyrek ID)
       Correlated ABYS_ID NOT EXISTS YASAK. */
    DECLARE @UseKeyList BIT = 0;
    IF @AGR_ID IS NOT NULL
       AND OBJECT_ID('tempdb..#MIG_INV_BATCH_KEYS') IS NOT NULL
       AND EXISTS (SELECT 1 FROM #MIG_INV_BATCH_KEYS)
        SET @UseKeyList = 1;

    IF OBJECT_ID('tempdb..#MIG_INV_KEYS') IS NOT NULL DROP TABLE #MIG_INV_KEYS;
    CREATE TABLE #MIG_INV_KEYS (ACTION_ID BIGINT NOT NULL PRIMARY KEY);

    IF @UseKeyList = 1
        INSERT INTO #MIG_INV_KEYS (ACTION_ID)
        SELECT DISTINCT CAST(k.ACTION_ID AS BIGINT)
        FROM #MIG_INV_BATCH_KEYS k
    ...
    IF OBJECT_ID('tempdb..#MIG_INV_SKIP') IS NOT NULL DROP TABLE #MIG_INV_SKIP;
    CREATE TABLE #MIG_INV_SKIP (ID BIGINT NOT NULL PRIMARY KEY);

    IF @UseKeyList = 1
    BEGIN
        INSERT INTO #MIG_INV_SKIP (ID)
        SELECT k.ACTION_ID
        FROM #MIG_INV_KEYS k
        WHERE EXISTS (
              SELECT 1
        ...
        WHERE EXISTS (
              ...
              WHERE t.ABYS_AGREEMENT_ID = @AGR_ID
                AND t.ABYS_ID = k.ACTION_ID
            )
          AND NOT EXISTS (SELECT 1 FROM #MIG_INV_SKIP x WHERE x.ID = k.ACTION_ID)
        OPTION (MAXDOP 8);
    END
    ELSE
    BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            ...
            TRY_CAST(s.READ_TRANSREF AS INT),
            ...
            TRY_CONVERT(UNIQUEIDENTIFIER, NULLIF(LTRIM(RTRIM(s.ETTN)), N'')),
        ...
        FROM (
            ...
            INNER JOIN #MIG_INV_KEYS kx ON kx.ACTION_ID = s0.ABYS_ACTION_ID
            WHERE (@AGR_ID IS NULL OR s0.ABYS_AGREEMENT_ID = @AGR_ID)
        ) s
        WHERE s.rn = 1
          AND NOT EXISTS (
              SELECT 1 FROM #MIG_INV_SKIP x WHERE x.ID = s.ABYS_ACTION_ID
          )
...
BEGIN
    ...
    -- @AGR_ID dolu: AGREEMENT_NUMBER ile cozum + fatura zinciri temizligi
    IF @AGR_ID IS NOT NULL AND (@RESUME = 0 OR @HARD_RESET = 1)
    BEGIN
    ...
    IF OBJECT_ID('tempdb..#MIG_INV_BATCH_KEYS') IS NOT NULL DROP TABLE #MIG_INV_BATCH_KEYS;
    CREATE TABLE #MIG_INV_BATCH_KEYS (ACTION_ID BIGINT NOT NULL PRIMARY KEY);
    ...
    BEGIN
        ...
        TRUNCATE TABLE #MIG_INV_BATCH_KEYS;
        ...
        ELSE
        BEGIN
            /* AGR: key-list (seyrek ID; Last+1 BETWEEN yasak) */
            INSERT INTO #MIG_INV_BATCH_KEYS (ACTION_ID)
        ...
        BEGIN
            SET @Msg = 'Batch ' + CAST(@BatchNo AS VARCHAR(10))
                + CASE WHEN @AGR_ID IS NULL THEN ' | ARITH '
                       ELSE ' | keys=' + CAST((SELECT COUNT(*) FROM #MIG_INV_BATCH_KEYS) AS VARCHAR(20)) + ' | '
                  END
        ...
        BEGIN CATCH
            ...
            IF OBJECT_ID('tempdb..#MIG_INV_BATCH_KEYS') IS NOT NULL
                TRUNCATE TABLE #MIG_INV_BATCH_KEYS;
