/* prodREADY_ENERGY / 21_597_WIRE
   ...
   v3: fallback xref = dbo.MIG_597_STG_PAY_XREF (#temp yok)
*/
USE energy;
GO
...
BEGIN
    ...
    /* Fallback: MAP yoksa — physical staging (tempdb CP1 collate yok).
       ABYS_ID ve LREF_HINT ayri UPDATE; AGR varsa once kaynak filtrele. */
    IF OBJECT_ID('dbo.MIG_597_STG_PAY_XREF', 'U') IS NULL
    BEGIN
        RAISERROR('MIG_597_STG_PAY_XREF yok — once 20_597_INSERT.sql DDL deploy.', 16, 1);
        RETURN;
    END
