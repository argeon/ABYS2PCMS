...
-- ABYS_ID : kaynak LREF (kopru; ORACLE_LINE_ID tekil DEGIL — ~155K cift)
-- ABYS_ORACLE_LINE_ID : ORACLE_LINE_ID (bilgi)
-- ABYS_LREF : kaynak LREF (ABYS_ID ile ayni)
-- ABYS_PROJECT_ID : ORACLE_PROJECT_ID → PROJECT wire
...
BEGIN
    ...
    ;WITH Required (COL_NAME) AS (
        ...
        FROM (VALUES
            ...
            ('OLD_PROJECT_STATUS'), ('TESISAT_NO'), ('PROJE_SATIR'), ('SERVIS_KUTU_NO')
        ) v(COL_NAME)
    )
...
SELECT
    ...
    CAST(s.ORACLE_PROJECT_ID AS BIGINT)                                     AS ABYS_PROJECT_ID,
    ...
    LEFT(s.WORKERID2, 40)                                                   AS ABYS_STG_WORKERID2,
    CAST(s.TRC_ENG_ID AS BIGINT)                                            AS ABYS_STG_TRC_ENG_ID,
    CAST(s.CTRL_ENG_ID AS BIGINT)                                           AS ABYS_STG_CTRL_ENG_ID,
    CAST(s.OLD_PROJECT_STATUS AS SMALLINT)                                  AS ABYS_STG_OLD_PROJECT_STATUS,
    TRY_CAST(s.TESISAT_NO AS DECIMAL(22, 0))                                AS ABYS_STG_TESISAT_NO,
    CAST(s.PROJE_SATIR AS BIGINT)                                           AS ABYS_STG_PROJE_SATIR,
    CAST(s.SERVIS_KUTU_NO AS BIGINT)                                        AS ABYS_STG_SERVIS_KUTU_NO,
    LEFT(s.SAP_TUKETIM_NOKTASI, 100)                                        AS ABYS_STG_SAP_TUKETIM_NOKTASI,
    LEFT(s.SAP_TESISAT_NO, 100)                                             AS ABYS_STG_SAP_TESISAT_NO,
